...
def threshold_demo(image_path):
    ...
    binary_c = np.expand_dims(binary, 2)
    return binary_c
...
def scence_batch_gen(batch_img_list, batch_img_label_list,
    ...
    for i, path, label in zip(range(batch_size), batch_img_list, batch_img_label_list):
        # 对图像进行放缩
        image=threshold_demo(path)

        img_size = image.shape #[H,W,C]
        height_ratio = image_shape[0] / img_size[0]
        if int(img_size[1] * height_ratio) > image_shape[1]:
            new_img_size = (image_shape[1], image_shape[0])  #image_shape [H,W,C]
            image = cv2.resize(image,new_img_size)
            image=np.expand_dims(image,2)
            image=image/255
            batch_images[i, :, :, :] = image
        else:
            new_img_size = (int(img_size[1] * height_ratio),image_shape[0])
            image=cv2.resize(image,new_img_size)
            image=np.expand_dims(image,2)
            image=image/255
